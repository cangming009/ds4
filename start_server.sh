...
基本选项 / Basic:
  ...
  -n, --tokens N        最大输出 tokens (默认: 393216)
...
磁盘 KV 缓存 / Disk KV Cache:
  ...
  --kv-cache-max-entries N  最大缓存文件数 (LRU, 0=不限, 默认: 10)
...
示例 / Examples:
  # 默认最佳启动 (Think Max + KV缓存 + 权重预热)
  ./start_server.sh

  # 自定义模型路径
  ./start_server.sh -m /path/to/model.gguf

  # 禁用缓存，使用近似内核
  ./start_server.sh --no-kv-cache --no-quality

  # 修改端口，暴露到局域网
  ./start_server.sh --port 8080 --host 0.0.0.0
EOF
    exit 0
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "使用 / Use: ./start_server.sh --help"
            exit 1 ;;
    esac
done
